...
useState<string[]>([]);
  ...
  const handleUpdateNutritionCard = (index: number, field: "label" | "value", val: string) => {
    setNutritionCards(prev => prev.map((card, i) => i === index ? { ...card, [field]: val } : card));
  };
    ...
    formData.append(
      ...
      file
        ...
        if (createdProductId) {
          ...
          variantData.append("productId", String(createdProductId));
        ...
        console.error(error);
        const serverMsg = error.response?.data?.message || "Failed to add product";
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full bg-black border border-white/10 focus:border-red-500 rounded-2xl px-5 py-4 outline-none text-white text-sm transition-colors"
              />
            </div>
              ...
              {nutritionCards.map((card, idx) => (
                <div key={idx} className="flex gap-4 items-center">
                  ...
                  <button
                    ...
                    className="p-3 text-red-500 hover:bg-red-500/10 rounded-full transition-colors cursor-pointer"
                  >
